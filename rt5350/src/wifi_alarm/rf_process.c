...
//	char buf[1024];
	...
	/* 判断下收到是否遥控器 */
	struct json_object *ykq_obj = json_object_object_get(alarm_conf_json, _STRING_Remote);

	if(ykq_obj == NULL)
		return NULL;

	char str[50];
	__json_object_object_foreach(ykq_obj,key,val)
	{
		//__json_key_to_string(ykq_obj, key, str, 50);
		//printf("key is [%s], code is [%s] \r\n", key, code);
		/* 前五个码数是否一致 */
		if(memcmp(code, key, 5) == 0)
		{
			printf("recv rf for Remote \r\n");
			type = 3;
			strcpy(type_str, _STRING_Remote);
			json_object_object_add(val, _STRING_ID, json_object_new_string(code));
			obj = val;
			goto out1;
		}
	}
